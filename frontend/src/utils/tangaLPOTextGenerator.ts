...
export const generateTangaLPOText = (lpo: TangaLPO): string => {
  const lines: string[] = [];
  lines.push(`LPO No. ${lpo.lpoNo},,, Date: ${new Date(lpo.date).toLocaleDateString('en-GB')}`);
  lines.push(`Yard: TANGA,,, Currency: ${lpo.currency}`);
...
export const copyTangaLPOText = (lpo: TangaLPO): Promise<boolean> =>
  copyToClipboard(generateTangaLPOText(lpo));

export const generateTangaLPOForWhatsApp = (lpo: TangaLPO): string => {
  const lines: string[] = [];
  lines.push(`*TANGA YARD LPO*`);
...
export const copyTangaLPOForWhatsApp = (lpo: TangaLPO): Promise<boolean> =>
  copyToClipboard(generateTangaLPOForWhatsApp(lpo));
