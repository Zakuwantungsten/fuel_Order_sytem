...
.lpo-rd .icon-btn-pending{border-color:#fde68a;background:#fffbeb;color:#b45309;}
.lpo-rd .icon-btn-pending:hover{background:#fef3c7;}
.dark .lpo-rd .icon-btn-pending{border-color:#92400e;background:rgba(120,53,15,0.25);color:#fcd34d;}
.dark .lpo-rd .icon-btn-pending:hover{background:rgba(120,53,15,0.4);}
...
const LPODetailForm: React.FC<LPODetailFormProps> = ({
  ...
  const [yardDraftEntries, setYardDraftEntries] = useState<YardDraftEntry[]>([]);
  const [yardDraftVersion, setYardDraftVersion] = useState(0);
  ...
  // Confirm before creating a pending return DO (PR####) from a row action
  const [pendingReturnConfirm, setPendingReturnConfirm] = useState<{
    open: boolean;
    index: number;
    truckNo: string;
    loading: boolean;
  ...
  const applyStoredDraft = (stored: StoredFormData) => {
    ...
    setYardDraftEntries(stored.yardEntries || []);
    setYardDraftVersion((v) => v + 1);
    setStationQuery(
  ...
  useEffect(() => {
    if (isOpen && !initialData) {
      const stored = loadFormFromStorage();
      const hasRegular = !!(stored?.formData.entries && stored.formData.entries.length > 0);
      const hasYard = !!(stored?.yardEntries && stored.yardEntries.some((e) => (e.truckNo || '').trim()));
      if (stored && (hasRegular || hasYard)) {
  ...
  useEffect(() => {
    ...
    const hasRegular = !!(formData.entries && formData.entries.length > 0);
    const hasYard = yardDraftEntries.some((e) => (e.truckNo || '').trim());
    if (!hasRegular && !hasYard) return;
  ...
  const fetchTruckData = useCallback(async (truckNo: string): Promise<TruckFetchResult> => {
    ...
    try {
      ...
      if ((selectedRecord as any).isLocked) {
        statusMessage += ' | Locked (pending admin config — manual entry allowed)';
      }
  ...
  const canOfferPendingGoing = (index: number) => {
    const entry = formData.entries?.[index];
    const autoFill = entryAutoFillData[index];
    if ((autoFill?.entryType || 'regular') !== 'regular') return false;
    if (!isPendingGoingCreateMonth()) return false;
    if ((entry?.truckNo?.length || 0) < 4) return false;
    return shouldOfferPendingGoingCreate({
      warningType: autoFill?.warningType,
   ...
   */
  const requestCreatePendingReturnDo = (index: number) => {
    const entry = formData.entries?.[index];
    const truckNo = formatTruckNumber((entry?.truckNo || '').trim());
  ...
  const confirmCreatePendingReturnDo = async () => {
    ...
    if (index < 0) return;
    setPendingReturnConfirm((prev) => ({ ...prev, loading: true }));
    try {
      await handleCreatePendingDo(index, 'return');
  ...
  const handleCreatePendingDo = async (index: number, kind: 'going' | 'return') => {
    ...
    try {
      ...
      const queued = result.allJourneys?.queued || [];
      const pendingMatch = createdPendingDo
        ? queued.find((q) => (q.goingDo || '').toUpperCase() === createdPendingDo.toUpperCase())
        : undefined;
      const selectedRecord =
        (kind === 'going' ? (pendingMatch || queued[0]) : null) || result.fuelRecord;
      const selectedType: 'active' | 'queued' =
        selectedRecord?.journeyStatus === 'queued' ? 'queued' : 'active';
      const selectedIndex =
        selectedType === 'queued'
          ? Math.max(0, queued.findIndex((q) => (q.id || q._id) === (selectedRecord?.id || selectedRecord?._id)))
          : -1;

      const direction: 'going' | 'returning' = kind === 'return' ? 'returning' : (autoFill?.direction || 'going');
      const doNumber = direction === 'going'
        ? (selectedRecord?.goingDo || result.goingDo)
        : (selectedRecord?.returnDo || result.returnDo || selectedRecord?.goingDo || result.goingDo);
      const returnDoMissing = isReturnDoMissing(selectedRecord?.returnDo || result.returnDo);
      const goingDestination = selectedRecord?.originalGoingTo || selectedRecord?.to || result.goingDestination;
      const destinationForAllocation = direction === 'going' ? goingDestination : (selectedRecord?.to || result.destination);
      ...
      setEntryAutoFillData(prev => ({
        ...
        [index]: {
          ...
          direction,
          loading: false,
          fetched: result.success || !!selectedRecord,
          fuelRecord: selectedRecord,
          fuelRecordId: selectedRecord?.id || selectedRecord?._id,
          goingDestination,
          returnDoMissing,
          warningType: (result.success || selectedRecord) ? null : result.warningType || null,
          warningMessage: result.message,
          allJourneys: result.allJourneys,
          selectedJourneyType: selectedType,
          selectedJourneyIndex: selectedIndex,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start sm:items-center justify-center z-50 p-0 sm:p-4" onClick={handleCancel}>
        ...
        <form onSubmit={handleSubmit} className="lpo-scroll flex-1 overflow-y-auto p-5 sm:p-[26px]">
          ...
          {isYardMode ? (
            <div className="mb-5">
              ...
              <YardEntriesTable
                key={`${formData.station}-${yardDraftVersion}`}
                ref={yardEntriesRef}
                ...
                return (
                  <div key={index} className={`border rounded-lg p-2 transition-colors ${
                    ...
                    <div className="flex items-center gap-2 pt-1.5 mt-1.5 border-t border-gray-200 dark:border-gray-600">
                      ...
                      {mobileReturnDoMissing && (autoFill.entryType || 'regular') === 'regular' && (
                        <button
                          type="button"
                          onClick={() => requestCreatePendingReturnDo(index)}
                          disabled={autoFill.loading}
                          title="Create pending return DO (PR####)"
                          className="flex-1 px-3 py-1.5 text-[11px] font-medium text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-900/20 hover:bg-amber-100 dark:hover:bg-amber-900/40 rounded-lg inline-flex items-center justify-center gap-1 transition-colors disabled:opacity-50"
                        >
                        ...
                        <tr key={index} className={`lpo-row ${
                            ...
                            <td className="px-3 py-3 max-w-[180px]">
                              <div className="flex flex-col gap-1 text-xs">
                                ...
                                {autoFill.fetched && autoFill.allJourneys && (
                                  <div className="flex items-center gap-1 flex-wrap text-[10px] text-gray-600 dark:text-gray-400">
                                ...
                                {autoFill.direction === 'returning' && autoFill.returnDoMissing && autoFill.fuelRecord && (
                                  <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 w-fit" title="No Return DO found in the fuel record — cannot submit. Create a pending return DO from the row actions or switch back to Going.">
                                    ⛔ No Return DO
                                  </span>
                                )}
                                {autoFill.fuelRecord && pendingDoStatusLabel(autoFill.fuelRecord) && (
                          ...
                          <td style={{ textAlign: 'right' }}>
                            <span className="inline-flex gap-1 justify-end">
                              ...
                              {/* Create pending return DO — same Clock treatment as Pending DOs in nav */}
                              {autoFill.direction === 'returning' &&
                                autoFill.returnDoMissing &&
                                autoFill.fuelRecord &&
      ...
      <div onClick={e => e.stopPropagation()}>
        <ConfirmModal
          open={pendingReturnConfirm.open}
          title="Create Pending Return DO?"
          message={`Create a temporary pending return DO (PR####) for truck ${pendingReturnConfirm.truckNo}? It will stay on the fuel record until a real EXPORT DO is linked.`}
          variant="warning"
          confirmLabel="Create pending return"
          cancelLabel="Cancel"
          loading={pendingReturnConfirm.loading}
          onConfirm={confirmCreatePendingReturnDo}
          onCancel={() => {
            if (pendingReturnConfirm.loading) return;
            setPendingReturnConfirm({ open: false, index: -1, truckNo: '', loading: false });
          }}
        />
      </div>
