...
interface DarLPOPrintProps {
  data: DarLPO;
  preparedBy?: string;
}

const DarLPOPrint = forwardRef<HTMLDivElement, DarLPOPrintProps>(({ data, preparedBy }, ref) => {
  const fontSize = '13px';
  const headerFontSize = '14px';
  const ROWS_PER_PAGE = 25;

  const pages = useMemo(() => {
    const allPages: DarLPOEntry[][] = [];
  ...
  const renderTableRow = (entry: DarLPOEntry, index: number) => {
              ...
              <div style={{ borderBottom: '2px solid #000', paddingBottom: '12px', marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000', margin: '0', letterSpacing: '0.3px' }}>
                    LPO No. {data.lpoNo} (Continued) — Dar Yard
                  </h2>
                  <div style={{ fontSize: '12px', color: '#555', fontWeight: '500' }}>
                    Date:{' '}
                    {new Date(data.date).toLocaleDateString('en-GB', {
              ...
              <tbody>
                ...
                {isLastPage && (
                  <tr style={{ backgroundColor: '#e8e8e8' }}>
                    ...
                    <td style={{ border: '1px solid #000', padding: '8px 6px', textAlign: 'center', verticalAlign: 'middle', lineHeight: '1.4' }} />
                    <td style={{ border: '1px solid #000', padding: '8px 6px', textAlign: 'center', verticalAlign: 'middle', fontWeight: 'bold', fontSize: headerFontSize, color: '#000', lineHeight: '1.4' }}>
...
DarLPOPrint.displayName = 'DarLPOPrint';

export default DarLPOPrint;
