...
import {
  ...
  Calendar, ChevronDown, Wallet, Image, Check, MessageSquare, Loader2
} from 'lucide-react';
import { toast } from 'react-toastify';
import type { LPOEntry, LPOSummary } from '../types';
import { useAuth } from '../contexts/AuthContext';
import { lpoDocumentsAPI } from '../services/api';
import { useDriverAccountEntries } from '../hooks/useLPOs';
...
const DriverAccountWorkbook: React.FC<DriverAccountWorkbookProps> = ({ onNavigateToSheet }) => {
  const { user } = useAuth();
  const { data: daEntries = [], isLoading: loading } = useDriverAccountEntries();
  ...
  const filteredEntries = daEntries.filter(entry => {
  ...
  useEffect(() => {
    if (loading || daEntries.length === 0) return;
    const now = new Date();
    ...
    const currentMonthAll = daEntries.filter(entry => {
  ...
  }, [daEntries, loading, availablePeriods, selectedPeriods]);

  useEffect(() => {
    if (!wasAutoFallback.current || loading) return;
    const now = new Date();
    const defYear = now.getFullYear(), defMonth = now.getMonth() + 1;
    const currentMonthHasData = daEntries.some(entry => {
      if (!entry.createdAt) return false;
      const d = new Date(entry.createdAt);
      return !isNaN(d.getTime()) && d.getFullYear() === defYear && d.getMonth() + 1 === defMonth;
    });
  ...
  const exportToExcel = async () => {
    ...
    const data = filteredEntries.map((entry, index) => ({
      ...
      'Destination': entry.destinations || 'NIL',
    ...
    for (let row = range.s.r; row <= range.e.r; row++) {
      for (let col = range.s.c; col <= range.e.c; col++) {
        ...
        ws[cellRef].s = row === 0
          ? { border: borderStyle, alignment: { horizontal: 'center', vertical: 'center' }, font: { bold: true }, fill: { fgColor: { rgb: 'FEE2E2' } } }
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'driver account entries');
    XLSX.writeFile(wb, 'DRIVER_ACCOUNT_ENTRIES.xlsx');
  };
      ...
      <div className="border-b dark:border-gray-700 px-4 sm:px-6 py-3 sm:py-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center space-x-3">
            <Wallet className="w-5 h-5 sm:w-6 sm:h-6 text-red-600" />
            <h1 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-gray-100">Driver Account</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
            ...
            <div>
              <input
                ...
                className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-red-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
              />
            </div>
                  ...
                  <div className="p-2 border-b border-gray-200 dark:border-gray-600 space-y-1">
                    {availablePeriods.some(p => p.year === now.getFullYear() && p.month === now.getMonth() + 1) && (
                      <button
                        ...
                        className="w-full text-left px-2 py-1 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded"
                      >
              ...
              {showStationDropdown && (
                <div className="absolute z-50 mt-1 w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg max-h-60 overflow-auto">
                  <button type="button" onClick={() => { setStationFilter(''); setShowStationDropdown(false); }} className="w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-900 dark:text-gray-100 flex items-center justify-between">
                    <span>All Stations</span>{stationFilter === '' && <Check className="w-4 h-4 text-red-600" />}
                  </button>
                  {availableStations.map(station => (
                    <button key={station} type="button" onClick={() => { setStationFilter(station); setShowStationDropdown(false); }} className="w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-900 dark:text-gray-100 flex items-center justify-between">
                      <span>{station}</span>{stationFilter === station && <Check className="w-4 h-4 text-red-600" />}
                    </button>
                  ))}
                </div>
              )}
            ...
            <input
              ...
              className="px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-red-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
            />
      ...
      <div className="flex-1 overflow-auto px-4 sm:px-6 pb-4">
        {filteredEntries.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-gray-500 dark:text-gray-400">
            <Wallet className="w-12 h-12 mb-3 text-red-300 dark:text-red-700" />
            <p>No driver account entries found</p>
            <p className="text-sm mt-1">Create driver account entries by typing "DA" in the DO field of the main LPO form</p>
          </div>
        ) : (
          <>
            {/* Mobile card view */}
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <span className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded text-[10px] font-bold">DA</span>
                      <div className="relative">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="p-1 text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 rounded hover:bg-red-50 dark:hover:bg-red-900/20"
                        >
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                    ...
                    <div><span className="text-gray-400 dark:text-gray-500">LPO: </span><span className="text-red-600 dark:text-red-400">{entry.lpoNo}</span></div>
                    <div><span className="text-gray-400 dark:text-gray-500">Dest: </span><span className="text-gray-700 dark:text-gray-300">{entry.destinations || 'NIL'}</span></div>
                  </div>
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border dark:border-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredEntries.map((entry, index) => (
                    <tr
                      key={entry.id || `${entry.lpoNo}-${index}`}
                      className={`transition-colors cursor-pointer ${entry.isCancelled ? 'bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30' : 'hover:bg-red-50 dark:hover:bg-red-900/10'}`}
                      onClick={() => {
                        if (onNavigateToSheet && entry.lpoNo) {
                          const entryDate = entry.createdAt ? new Date(entry.createdAt) : new Date();
                          const year = !isNaN(entryDate.getTime()) ? entryDate.getFullYear() : new Date().getFullYear();
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-xs font-medium text-red-600 dark:text-red-400">
                      ...
                      <td className="px-3 py-2 text-xs text-gray-900 dark:text-gray-100">{entry.destinations || 'NIL'}</td>
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400" onClick={(e) => e.stopPropagation()}>
                        <div className="relative">
                          <button
                            ...
                            className="flex items-center px-2 py-1 text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded"
...
export default DriverAccountWorkbook;
