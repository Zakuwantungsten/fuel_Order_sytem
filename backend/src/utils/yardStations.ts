 ...
 * Not FuelStationConfig rows — form/list treat them like CASH/CUSTOM.
 */
...
export type YardFuelField = 'tangaYard' | 'darYard';
...
export function yardFuelFieldForStation(station: string | null | undefined): YardFuelField | null {
  if (isTangaYardStation(station)) return 'tangaYard';
  if (isDarYardStation(station)) return 'darYard';
  return null;
}
