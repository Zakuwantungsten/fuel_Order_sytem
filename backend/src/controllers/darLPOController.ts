...
export const createDarLPO = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  await AuditService.log({
    ...
    resourceId: lpoNo,
    details: `Dar Yard LPO ${lpoNo} created on LPOSummary (${lpo.entries.length} entries${linked ? `, ${linked} fuel-linked` : ''}) by ${req.user?.username}`,
    ipAddress: req.ip,
    severity: 'medium',
  });
  ...
  res.status(201).json({
    success: true,
    message: linked > 0
      ? `Dar LPO created — ${linked} ${linked === 1 ? 'entry' : 'entries'} linked to fuel`
      : 'Dar LPO created successfully',
    data: responseData,
    linkResults,
  });
...
export const unlinkDarEntry = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  await AuditService.log({
    ...
    resourceType: resolved.source === 'legacy' ? 'DarLPODocument' : 'LPOSummary',
    resourceId: lpo.lpoNo,
    details: `Entry ${entryId} in Dar LPO ${lpo.lpoNo} unlinked — reversed ${result.reversedLiters}L from FuelRecord ${result.oldFuelRecordId} by ${username}`,
    ipAddress: req.ip,
    severity: 'medium',
  });
  ...
  res.status(200).json({
    success: true,
    message: `Entry unlinked — ${result.reversedLiters}L reversed from darYard`,
    data: tagYardDoc(lpo, resolved.source, resolved.station),
    meta: result,
  });
 ...
 */
export const relinkDarEntryIdentity = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  // Edit form always sends dest — apply after move so user override wins over FR dest.
  if (dest != null) {
  ...
  await AuditService.log({
    ...
    resourceType: resolved.source === 'legacy' ? 'DarLPODocument' : 'LPOSummary',
    resourceId: lpo.lpoNo,
    details: `Entry ${entryId} in Dar LPO ${lpo.lpoNo} identity relink (${move.status}) DO/truck → ${nextDo}/${nextTruck} by ${username}`,
    ipAddress: req.ip,
    severity: 'medium',
  });
