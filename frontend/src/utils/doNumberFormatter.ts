/**
 * Utility functions for DO number formatting (Frontend)
...
/**
 * Display DO number with optional prefix (DO/SDO)
 * @param doNumber - The DO number
 * @param doType - Optional DO type ('DO' or 'SDO')
 * @returns Formatted display string (e.g., "DO-0001/26")
 */
export function displayDONumber(doNumber: string, doType?: 'DO' | 'SDO'): string {
  if (!doNumber) return '';
  if (doType) {
    return `${doType}-${doNumber}`;
  }
  return doNumber;
}
